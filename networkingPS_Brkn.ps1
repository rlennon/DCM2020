...
foreach-object -Begin {("")} -Process { $k = [regex]::split($_,'='); if(($k[0].CompareTo("") -ne 0) -and ($k[0].StartsWith("[") -ne $True)) { $settings.Add($k[0], $k[1]) } }
Get-Content -path "C:\Users\Administrator\Desktop\OOPR\Settings.ini"
$computerNames = Get-Content -path C:\Users\Administrator\Desktop\OOPR\ipaddress.txt


#Calling the Main function to carry out network tests
Test-connection $computerNames
...
function Test-Connection
{
    ...
    $portList = $settings.PortsToValidate
    $Array = "$portlist".Split(",")
    

    # Start to write to the Log File. All output will be written in the Log File
    Start-Transcript -Path C:\Users\Administrator\Desktop\OOPR\LogFile.txt
...
function get-Check-OpenPorts
{
    ...
    try
    {
        ...
        netstat -an
        {
            ...
            Test-NetConnection ad1 -port 445|Format-List * -ErrorAction SilentlyContinue
