    ...
    Process {
        ...
        Foreach ($computerName in $computerNames) {
            ...
            if (Test-NetConnection -ComputerName $computerName -Count 1 -Quiet) { 
                ...
                $errorOutputArray += CheckWarningsErrors $computerName

                # Get Network Information
                $networkInformationArray += Get-NetworkInfo $computerName

                # Check for open ports as per list given
                $checkOpenPortsArray += CheckOpenPorts $computerName $portList
...
<#
.Synopsis
  Check OpenPorts function

.DESCRIPTION
   This function will get details about open ports

.PARAMETERS
    $ComputerName: A Valid Computer Name or IP Address  
    $PortList:     A list of the ports available
#>
