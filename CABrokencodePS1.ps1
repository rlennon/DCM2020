...
function NetworkTests
{
    ...
    # Uncomment the correct one of the above choices! (write -output chosen over write host as its listed on line 118 )
...
function Get-NetworkInfo
{
    ...
    Param( [string]$ComputerName = $env:computername)
        ...
        #a parameter should be added here for the string variable named ComputerName (Fixed as per query )
...
<#
.Synopsis
  Testing for any open ports * added to adddress below 
   
.DESCRIPTION
  Function used to check for open ports * 
   
.PARAMETERS  
  parameter is $ComputerName which a correct computer name or IP  * 
#>
...
function CheckOpenPorts
{
    ...
    try
    {
        ...
        foreach ($port in $portlist) { code added to address above query

        {
            
            #BSc DCM 2020 - Fix this (added the port and computer with the warning action) 
