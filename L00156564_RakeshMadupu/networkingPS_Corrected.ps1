...
.CONCLUSION
  ...
  get the current user logged onto the server, check if any security warnings and errors on the serverís
  event logs, display the serverís detailed network information and check if any given ports are open or
...
$ServerNames = Get-Content $settings.Get_Item("IPAddressesFile")
#Calling the Main function to carry out network tests
Network-Tests $ServerNames
...
function Network-Tests
{
   ...
   $portinfo = $settings.PortsToValidate.Split(",")
