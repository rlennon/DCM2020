...
Test-NetConnection $computerNames
...
function Get-WarningsErrors
{
    ...
    try
    {
        ...
        If ($null = $EventLogTest -ne $_result)
        {
...
.DESCRIPTION
    ...
    This function will test for open ports by computer name
...
function Test-OpenPorts
{
    ...
    try
    {
        ...
        {foreach ($ports in $portList)
          {  
            #BSc DCM 2020 - Fix this
             $portConnected = Test-NetConnection -ComputerName $ComputerName -port $port -InformationLevel "Detailed" -Warning SilentlyContinue
